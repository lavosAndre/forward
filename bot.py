...
def move_messages(context: CallbackContext) -> None:
    ...
    if messages and len(messages) == 100:
        ...
        context.job_queue.run_once(lambda c: move_messages(c), when=2)  # Wait 2 seconds before the next batch
...
def main() -> None:
    ...
    updater.job_queue.run_once(lambda c: move_messages(c), when=0)
